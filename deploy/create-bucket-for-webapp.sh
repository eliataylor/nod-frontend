...
required_vars=("GCP_PROJECT_ID" "GCP_BUCKET_NAME" "GCP_BUCKET_LOCATION" "STATIC_IP_NAME" "SSL_CERT_NAME" "GCP_DNS_ZONE_NAME" "DOMAIN_NAME")
...
SANITIZED_BUCKET_NAME=$(sanitize_bucket_name "$GCP_BUCKET_NAME")
...
if ! gcloud compute addresses describe $SANITIZED_BUCKET_NAME-ip --global > /dev/null 2>&1; then
  ...
  echo -e "\e[31mIP address $SANITIZED_BUCKET_NAME-ip already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute ssl-certificates describe $SSL_CERT_NAME --global > /dev/null 2>&1; then
  ...
  echo -e "\e[31mSSL certificate $SSL_CERT_NAME already exists. Skipping creation.\e[0m"
fi
...
progress "$task_name"
if ! gcloud storage buckets describe gs://$GCP_BUCKET_NAME --format="json(name)" > /dev/null 2>&1; then
  # Create GCS Bucket
...
if ! gcloud compute backend-buckets describe $SANITIZED_BUCKET_NAME-backend > /dev/null 2>&1; then
  ...
  echo -e "\e[31mBackend bucket $SANITIZED_BUCKET_NAME-backend already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute url-maps describe $SANITIZED_BUCKET_NAME-url-map > /dev/null 2>&1; then
  ...
  echo -e "\e[31mURL map $SANITIZED_BUCKET_NAME-url-map already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute target-https-proxies describe $SANITIZED_BUCKET_NAME-https-proxy > /dev/null 2>&1; then
  ...
  echo -e "\e[31mHTTPS proxy $SANITIZED_BUCKET_NAME-https-proxy already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute forwarding-rules describe $SANITIZED_BUCKET_NAME-https-rule --global > /dev/null 2>&1; then
  ...
  echo -e "\e[31mHTTPS forwarding rule $SANITIZED_BUCKET_NAME-https-rule already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute url-maps describe http-to-https --global > /dev/null 2>&1; then
  ...
  echo -e "\e[31mURL map http-to-https already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute target-http-proxies describe $SANITIZED_BUCKET_NAME-http-proxy --global > /dev/null 2>&1; then
  ...
  echo -e "\e[31mHTTP proxy $SANITIZED_BUCKET_NAME-http-proxy already exists. Skipping creation.\e[0m"
fi
...
if ! gcloud compute forwarding-rules describe $SANITIZED_BUCKET_NAME-http-rule --global > /dev/null 2>&1; then
  ...
  echo -e "\e[31mHTTP forwarding rule $SANITIZED_BUCKET_NAME-http-rule already exists. Skipping creation.\e[0m"
fi
